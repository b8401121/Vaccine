...
pub struct VaccineItem {    pub name: String,
    pub dose_info: String,
    pub timing_info: String,
    pub category: String, // "Routine" (公費常規) | "SelfPaid" (自費建議) | "Subsidized" (縣市補助)
    pub description: String,
    pub audience: String,
}

#[derive(Serialize, Deserialize, Clone)]
pub struct TimelineMilestone {    pub title: String,
    pub age_months: i32,
    pub target_date: String,
    pub status: String, // "Past", "Current", "Next"
    pub vaccines: Vec<VaccineItem>,
    pub co_admin_guide: Vec<String>,
}

#[derive(Serialize, Deserialize, Clone)]
pub struct VaccineResponse {    pub age_display: String,
    pub child_age_detail: String,
    pub gender_display: String,
    pub location_display: String,
    pub current_visit_date: String,
    pub current_visit_milestone: String,
    pub next_visit_date: String,
    pub next_visit_milestone: String,
    pub milestones: Vec<TimelineMilestone>,
}

#[derive(Serialize, Deserialize, Clone)]
pub struct VaccineDetailDoc {    pub id: String,
    pub name: String,
    pub aliases: String,
    pub category: String, // "Routine" | "SelfPaid" | "Both"
    pub target_audience: String, // "兒童/青少年" | "成人/長者" | "全齡通用"
    pub prevent_disease: String,
    pub full_description: String,
    pub schedule: Vec<String>,
    pub notes: String,
}

pub struct MilestoneSpec {    pub title: &'static str,
    pub min_month: i32,
    pub max_month: i32,
    pub vaccines: Vec<VaccineItem>,
}
...
fn get_co_admin_guide(min_month: i32, is_adult: bool, is_travel: bool) -> Vec<String> {
    if is_travel {
        return vec![
            ...
            "🍂 流感疫苗同次建議：出國前建議同時施打 1 劑季節性流感疫苗（不活化針劑可於不同手臂同時接種）。".into(),
    ...
    if is_adult {
        return vec![
            ...
            "🍂 50-64歲熟齡長者首選佐劑加強型流感疫苗(東洋輔流禦)；65歲以上銀髮長者首選高劑量(賽諾菲菲優達)或佐劑加強型。".into(),
        ];
    }

    match min_month {
        0 => vec![
            "💉 施打部位：B型肝炎第1劑 (不活化) 於大腿前外側肌肉施打。".into(),
            "💡 高風險提醒：母親若為 B 肝帶原者，新生兒需於 24 小時內於另一大腿部位加打 1 劑 B 肝免疫球蛋白 (HBIG)。".into(),
            "⚠️ 注意：新生兒未滿 6 個月，不符合流感疫苗施打適應症。".into(),
        ],
        1 => vec![
        ...
        2 => vec![
            ...
            "💉 右大腿：13 價結合型肺炎鏈球菌 PCV13 第 1 劑 (不活化)".into(),
            "🍼 口服減毒：輪狀病毒疫苗 第 1 劑 (口服給藥，與大腿針劑不衝突，建議於注射前或安撫後口服投藥)".into(),
            "💡 自費腸病毒 EV71：可同次施打於另一大腿部位 (與同腿針劑間隔至少 2.5cm)，或擇日施打。".into(),
            "⚠️ 流感提示：未滿 6 個月嬰兒尚未達流感疫苗施打年齡，建議照顧同住家長施打形成包繭保護圈。".into(),
        ],
        4 => vec![
            "💉 左大腿：五合一疫苗 第 2 劑 (不活化)".into(),
            "💉 右大腿：13 價結合型肺炎鏈球菌 PCV13 第 2 劑 (不活化)".into(),
            "🍼 口服減毒：輪狀病毒疫苗 第 2 劑 (口服投藥，與肢體針劑完全無干擾)".into(),
            "💡 自費腸病毒 EV71：可同次施打或擇日完成。".into(),
            "⚠️ 流感提示：未滿 6 個月嬰兒尚未達流感疫苗施打年齡，同住照顧者請每年按時接種流感疫苗。".into(),
        ],
        5 => vec![
        ...
        6 => vec![
            ...
            "🍂 秋冬流感首選：寶寶已滿 6 個月！強烈建議同次於『另一側大腿部位』(距同側針劑至少 2.5cm) 或分左右腿追加『季節性流感疫苗(標準四價細胞/雞胚型)』。".into(),
            "🍼 口服減毒：輪狀病毒疫苗第 3 劑 (若選用 3 劑型廠牌，口服與不活化針劑同次完成，完全不互斥)。".into(),
            "⚠️ 初次接種提醒：8 歲以下初次施打流感疫苗需間隔至少 4 週接種 2 劑，方能建立完整免疫力。".into(),
        ],
        12 => vec![
            "💉 左手臂/大腿：麻疹腮腺炎德國麻疹 MMR 混合疫苗 第 1 劑 (活性減毒)".into(),
            "💉 右手臂/大腿：水痘疫苗 Varicella 第 1 劑 (活性減毒)".into(),
            "💉 大腿部位：13 價肺炎鏈球菌 PCV13 第 3 劑追加劑 (不活化)".into(),
            "🍂 季節性流感疫苗：為不活化疫苗，可於同次同天與 MMR、水痘、PCV13 分開不同肢體部位同時施打！".into(),
            "⚠️ 活性減毒間隔金律：MMR 與水痘若未在同一天完成施打，兩者必須間隔至少 28 天 (4 週) 以上方可施打另一支！".into(),
            "💡 自費 A 肝疫苗：不活化疫苗，可同次施打於不同部位或間隔任意時間。".into(),
        ],
        15 => vec![
            "💉 左手臂/大腿：日本腦炎疫苗 第 1 劑 (活性減毒)".into(),
            "🍂 季節性流感疫苗：不活化流感疫苗可與日本腦炎活性疫苗於同天不同部位同時接種；若非同天施打，不活化與活性疫苗無間隔限制。".into(),
        ...
        18 => vec![
            ...
            "🍂 季節性流感疫苗：不活化疫苗，可同次於不同肢體部位同時追加施打，形成全面秋冬防護！".into(),
            "✅ 不活化疫苗黃金組合：五合一、A肝與流感疫苗均為不活化疫苗，同次分開左右肢體接種安全又省時。".into(),
        ],
        27 => vec![
            "💉 左手臂：日本腦炎疫苗 第 2 劑 (活性減毒，與第1劑隔12個月)".into(),
            "💉 右手臂：A 型肝炎疫苗 第 2 劑 (不活化，與第1劑隔6個月)".into(),
            "🍂 季節性流感疫苗：滿 2 歲以上可選注射型或自費 AZ 能伏鼻(FluMist 鼻噴免打針)。針劑型可與日腦/A肝同天不同部位施打；若選鼻噴式(活性減毒)，與活性日腦建議同天施打或間隔 28 天。".into(),
        ],
        48 => vec![
            "💉 自費追加水痘第 2 劑 (活性減毒，小兒感染症醫學會建議 4-6 歲追加)".into(),
            "🍂 季節性流感疫苗：滿 4 歲可選標準針劑或 AZ 能伏鼻(鼻噴免打針)。針劑型與水痘可同天分左右手臂施打；若選鼻噴式(活性)，與水痘建議同天施打或間隔至少 28 天。".into(),
        ],
        60 => vec![
            "💉 左上臂三角肌：四合一疫苗 DTaP-IPV 追加劑 (不活化)".into(),
            "💉 右上臂三角肌：MMR 混合疫苗 第 2 劑 (活性減毒)".into(),
            "🍂 季節性流感疫苗：入學前強烈建議完成流感防護！針劑型可同天分開部位施打；害怕打針者可選 AZ 能伏鼻(鼻噴免打針，若同天噴鼻無須扎針)。".into(),
            "✅ 國小入學前疫苗黃金時程：建議於幼兒園大班升國小暑假同次完成四合一、MMR與流感疫苗！".into(),
        ],
        _ => vec![
            "💉 不活化疫苗同次原則：流感、五合一、肺炎鏈球菌、B肝、A肝等不活化針劑，均可於同一次就診分開不同肢體部位同時施打。".into(),
            "🍼 口服減毒疫苗 (如輪狀病毒)：口服投藥經腸道吸收，與任何注射針劑均不互相干擾，可在針劑施打前後順利服用。".into(),
            "👃 鼻噴型流感疫苗 (AZ 能伏鼻 FluMist)：為活性減毒黏膜噴劑，免打針無痛；若與其他活性減毒注射疫苗(如水痘、MMR、活性日腦)非同天施打，需間隔至少 28 天。".into(),
            "⚠️ 活性減毒針劑間隔原則：MMR、水痘、活性日本腦炎等活性減毒針劑，若未於同天施打，需間隔至少 28 天 (4 週) 以上。".into(),
        ],
    }
}


pub fn get_eligible_vaccines_core(
    year: i32,
    month: u32,
    day: u32,
    is_roc: bool,
    ...
    let mut child_specs = vec![
        ...
        MilestoneSpec {
            ...
            vaccines: vec![
                ...
                VaccineItem {
                    name: "季節性流感疫苗 (2026最新規格)".into(),
                    dose_info: "第 1-2 劑 (未滿9歲初次接種隔4週打2劑)".into(),
                    timing_info: "滿 6 個月以上 (每年秋冬流感季)".into(),
                    category: "Routine".into(),
                    description: "【6個月~未滿2歲】建議標準型：東洋 輔流威護(MDCK細胞培養/無蛋)、GSK 伏流感(雞胚)、賽諾菲 菲流達(雞胚)。8歲以下初次接種需打2劑，間隔4週。".into(),
                    audience: "Children".into(),
                },
                VaccineItem {
                    name: "輪狀病毒疫苗 (Rotavirus)".into(),
        ...
        MilestoneSpec {
            ...
            vaccines: vec![
                ...
                VaccineItem {
                    name: "季節性流感疫苗 (2026最新規格)".into(),
                    dose_info: "每年 1 劑 (未滿9歲初次接種隔4週打2劑)".into(),
                    timing_info: "每年秋冬流感季".into(),
                    category: "Routine".into(),
                    description: "【2歲~未滿3歲幼童】一般接種：東洋輔流威護(細胞培養)、GSK伏流感、賽諾菲菲流達。◆ 害怕打針者首選：自費 AZ 能伏鼻 (FluMist 鼻噴式免打針活性減毒，建立第一線黏膜防禦，注意氣喘喘鳴禁忌)。".into(),
                    audience: "Children".into(),
                },
            ],
        },
        ...
        MilestoneSpec {
            ...
            vaccines: vec![
                ...
                VaccineItem {
                    ...
                    timing_info: "每年秋冬流感季 (3歲以上至學生)".into(),
                    category: "Routine".into(),
                    description: "【3歲~17歲兒童青少年】一般接種：國光安定伏、高端福喜健、東洋輔流威護、GSK伏流感、賽諾菲菲流達。◆ 害怕打針者首選：自費 AZ 能伏鼻 (FluMist 鼻噴式免打針)。".into(),
                    audience: "Children".into(),
                },
            ],
        },
    ...
    if age_years >= 18 {
        let flu_item = if age_years >= 65 {
            VaccineItem {
                name: "季節性流感疫苗 (2026長者加強型首選)".into(),
                dose_info: "每年 1 劑".into(),
                timing_info: "秋冬流感季 (65歲以上長者)".into(),
                category: "Routine".into(),
                description: "【65歲以上銀髮長者首選】：強烈建議選用加強型以克服免疫老化！首選【高劑量加強型 賽諾菲菲優達 (Efluelda High-Dose，含4倍抗原，重症保護力提升24.2%)】或【含佐劑加強型 東洋輔流禦 (Fluad)】。安養長照長者公費優先，一般長者可自費預約。亦可選一般標準型。".into(),
                audience: "Adults".into(),
            }
        } else if age_years >= 50 {
            VaccineItem {
                name: "季節性流感疫苗 (2026熟齡加強型推薦)".into(),
                dose_info: "每年 1 劑".into(),
                timing_info: "秋冬流感季 (50-64歲熟齡/免疫低下)".into(),
                category: "Routine".into(),
                description: "【50~64歲熟齡與免疫低下推薦】：建議選用【含佐劑加強型 東洋輔流禦 (Fluad)】，添加 MF59 專利佐劑能刺激更強且持久的抗體保護力。亦可選擇標準型（東洋輔流威護、GSK伏流感、賽諾菲菲流達、國光安定伏、高端福喜健）。".into(),
                audience: "Adults".into(),
            }
        } else {
            VaccineItem {
                name: "季節性流感疫苗 (2026成人標準防護)".into(),
                dose_info: "每年 1 劑".into(),
                timing_info: "秋冬流感季 (18-49歲一般成人)".into(),
                category: "Routine".into(),
                description: "【18~49歲青壯年】：可選擇標準型疫苗（東洋 輔流威護【細胞培養/無蛋】、GSK 伏流感、賽諾菲 菲流達、國光 安定伏、高端 福喜健）。公費資格者按時程施打，非公費者建議自費接種形成群體防護。".into(),
                audience: "Adults".into(),
            }
        };
...
pub fn get_all_vaccines_core() -> Vec<VaccineDetailDoc> {
    vec![
        VaccineDetailDoc {
        ...
        VaccineDetailDoc {
            id: "flu".into(),
            name: "【2026流感全攻略】8大廠牌規格與年齡選用指南".into(),
            aliases: "2026流感疫苗懶人包 / 8大廠牌 / 細胞培養 / 佐劑加強 / 高劑量 / 鼻噴免打針".into(),
            category: "Both".into(),
            target_audience: "全齡通用 (6個月以上至長者)".into(),
            prevent_disease: "季節性 A型 (H1N1/H3N2) 與 B型流感、重症肺炎與住院死亡".into(),
            full_description: "2026年台灣市面共有8款流感疫苗，涵蓋4大核心技術：傳統雞胚培養、MDCK細胞培養(無蛋蛋白)、MF59佐劑加強型(50歲+/免疫低下)、4倍抗原高劑量型(65歲+)與免打針鼻噴式(2-17歲)。公費疫苗按政府批次配送，若需特定優質廠牌建議自費預約。".into(),
            schedule: vec![
                "◆ 滿6個月~未滿9歲初次接種：施打 2 劑 (間隔 4 週)".into(),
                "◆ 9歲以上至成人長者：每年秋冬施打 1 劑".into(),
                "◆ 2歲~未滿18歲怕打針者：可選自費 AZ 能伏鼻 (鼻噴給藥)".into(),
                "◆ 50歲以上或免疫低下者：推薦東洋 輔流禦 (MF59佐劑加強型)".into(),
                "◆ 65歲以上銀髮長者：推薦賽諾菲 菲優達 (4倍高劑量加強型)".into(),
            ],
            notes: "雞蛋過敏者可安心施打，亦可優先選用東洋輔流威護(細胞培養)。流感疫苗可與新冠、肺炎鏈球菌疫苗同日不同部位同時施打。".into(),
        },
        VaccineDetailDoc {
            id: "flu_flucelvax".into(),
            name: "東洋 輔流威護 四價流感疫苗 (Flucelvax Tetra)".into(),
            aliases: "輔流威護 / 東洋細胞流感 / MDCK 細胞培養流感疫苗 / CSL Seqirus".into(),
            category: "Both".into(),
            target_audience: "滿 6 個月以上全齡通用 (嬰幼兒、孕婦、成人、長者)".into(),
            prevent_disease: "季節性 A/B 型流感病毒感染及重症".into(),
            full_description: "【MDCK犬腎細胞培養技術】完全不經過受精雞胚培養，零雞蛋蛋白殘留，徹底杜絕病毒在雞蛋培養過程中發生的突變適應，WHO臨床證實抗原吻合度最高！適合重度雞蛋過敏者或追求精準保護力之民眾。".into(),
            schedule: vec![
                "滿6個月以上~未滿9歲且初次接種：施打 2 劑 (間隔 4 週)".into(),
                "9歲以上與成人：每年接種 1 劑 (公費依配送/自費可指定)".into(),
            ],
            notes: "公自費皆有供應。注射部位偶有輕微紅腫疼痛，多於2-3天內緩解。".into(),
        },
        VaccineDetailDoc {
            id: "flu_fluad".into(),
            name: "東洋 輔流禦 佐劑加強型四價流感疫苗 (Fluad Tetra)".into(),
            aliases: "輔流禦 / Fluad / MF59佐劑流感疫苗 / 熟齡長者加強型".into(),
            category: "Both".into(),
            target_audience: "50 歲以上熟齡長者 / 服用免疫抑制劑者 / 免疫功能低下者".into(),
            prevent_disease: "50歲以上族群季節性流感重症、肺炎住院及併發症".into(),
            full_description: "【專利 MF59 免疫佐劑】添加專利角鯊烯水包油乳化佐劑，可大幅活化抗原呈現細胞與T/B淋巴細胞，專門克服長者與慢性病患之「免疫老化 (Immunosenescence)」，抗體保護力顯著增強且持久長達 12 個月！公費優先提供安養及長照機構65歲以上長者。".into(),
            schedule: vec![
                "50歲以上成人：每年秋冬接種 1 劑 (肌肉注射 0.5mL)".into(),
                "安養長照機構65歲以上長者享公費；一般50歲以上民眾可自費預約".into(),
            ],
            notes: "適應症為50歲以上。局部紅腫酸痛比例略高於一般劑型，屬正常免疫反應。".into(),
        },
        VaccineDetailDoc {
            id: "flu_efluelda".into(),
            name: "賽諾菲 菲優達 高劑量四價流感疫苗 (Efluelda High-Dose)".into(),
            aliases: "菲優達 / 賽諾菲高劑量流感 / Efluelda / Fluzone High-Dose".into(),
            category: "Both".into(),
            target_audience: "65 歲以上銀髮長者專用".into(),
            prevent_disease: "高齡長者流感病毒感染、重症肺炎、心血管併發症及住院".into(),
            full_description: "【4倍抗原高劑量加強型】每劑每株含有 60µg 血球凝集素 (HA)，總抗原量為一般標準疫苗 (15µg) 的 4 倍！大規模國際臨床實證，針對 65 歲以上長者之流感重症預防效果較標準型顯著提升 24.2%，大幅降低長者心肺併發症與住院率！".into(),
            schedule: vec![
                "65歲以上長者：每年秋冬接種 1 劑 (肌肉注射 0.7mL)".into(),
                "安養長照機構65歲以上長者公費配發；一般社區長者可自費預約施打".into(),
            ],
            notes: "專為65歲以上長者設計，未滿65歲不適用。".into(),
        },
        VaccineDetailDoc {
            id: "flu_flumist".into(),
            name: "AZ 能伏鼻 鼻噴式活性減毒流感疫苗 (FluMist / Fluenz Tetra)".into(),
            aliases: "能伏鼻 / FluMist / 鼻噴流感疫苗 / 免打針流感疫苗 / 阿斯特捷利康".into(),
            category: "SelfPaid".into(),
            target_audience: "2 歲至未滿 18 歲兒童與青少年 (全自費)".into(),
            prevent_disease: "兒童與青少年流感感染、呼吸道併發症及群聚傳播".into(),
            full_description: "【唯一免打針無痛流感疫苗】經雙側鼻孔黏膜各噴入 0.1mL 活性減毒疫苗。模仿病毒自然感染途徑，在鼻咽呼吸道黏膜直接刺激產生強效第一線分泌型 IgA 黏膜抗體與全身性細胞免疫！免除針頭恐懼，是兒童與青少年自費首選。".into(),
            schedule: vec![
                "2歲至未滿9歲初次接種流感疫苗者：間隔至少 4 週噴入 2 劑".into(),
                "曾接種過流感疫苗或9歲以上：每年施打 1 劑 (雙鼻孔各0.1mL)".into(),
            ],
            notes: "⚠️【禁忌症 (不可接種)】：懷孕婦女、嚴重免疫缺陷者、長期服用阿斯匹靈(Aspirin)之兒少、重度氣喘或近4週內曾有喘鳴(wheezing)發作者。全自費，無公費。".into(),
        },
        VaccineDetailDoc {
            id: "flu_fluarix".into(),
            name: "GSK 伏流感 四價流感疫苗 (Fluarix Tetra)".into(),
            aliases: "伏流感 / GSK 流感疫苗 / 葛蘭素史克".into(),
            category: "Both".into(),
            target_audience: "滿 6 個月以上全齡通用 (嬰幼兒、學童、成人、長者)".into(),
            prevent_disease: "A 型與 B 型流感重症與併發症".into(),
            full_description: "【傳統雞胚培養裂解疫苗】由全球疫苗領導大廠葛蘭素史克 (GSK) 於德國/歐洲原廠生產製造。全球使用量大、臨床研究歷史悠久，安全性與有效性廣受各國衛生當局認證。".into(),
            schedule: vec![
                "滿6個月至未滿9歲初次接種：施打 2 劑 (間隔 4 週)".into(),
                "9歲以上與成人：每年施打 1 劑".into(),
            ],
            notes: "公費常規主力廠牌之一，自費門診亦常備。".into(),
        },
        VaccineDetailDoc {
            id: "flu_vaxigrip".into(),
            name: "賽諾菲 菲流達 四價流感疫苗 (Vaxigrip Tetra)".into(),
            aliases: "菲流達 / 賽諾菲流感疫苗 / 巴斯德流感疫苗".into(),
            category: "Both".into(),
            target_audience: "滿 6 個月以上全齡通用 (嬰幼兒、孕婦、成人、長者)".into(),
            prevent_disease: "A 型 (H1N1/H3N2) 及 B 型流感病毒引起的流行性感冒".into(),
            full_description: "【法國賽諾菲原裝進口】歐洲巴斯德研發中心專業製造，為台灣歷年公費採購與小兒自費門診極為信賴之標準流感疫苗廠牌，孕婦及6個月大以上嬰兒均可安心接種。".into(),
            schedule: vec![
                "滿6個月至未滿9歲初次接種：施打 2 劑 (間隔 4 週)".into(),
                "9歲以上與成人：每年施打 1 劑".into(),
            ],
            notes: "公費常規主力廠牌之一，自費可指定。".into(),
        },
        VaccineDetailDoc {
            id: "flu_adimflu".into(),
            name: "國光生技 安定伏 裂解型四價流感疫苗 (AdimFlu-S)".into(),
            aliases: "安定伏 / 國光流感疫苗".into(),
            category: "Both".into(),
            target_audience: "滿 3 歲以上幼童、學童、青少年、成人與長者".into(),
            prevent_disease: "預防 A/B 型流行性感冒病毒引發之感染與重症".into(),
            full_description: "【台灣國產流感疫苗主力】由國光生物科技於台灣在地 PIC/S GMP 廠製造，供應台灣公費防疫計畫逾半數劑量。技術成熟、品質穩定，適用於 3 歲以上民眾。".into(),
            schedule: vec![
                "滿3歲至未滿9歲且初次接種：施打 2 劑 (間隔 4 週)".into(),
                "9歲以上至成人長者：每年施打 1 劑".into(),
            ],
            notes: "注意：本品適應症為滿 3 歲以上，未滿 3 歲嬰幼兒請選用6個月適應症之廠牌。".into(),
        },
        VaccineDetailDoc {
            id: "flu_gcflu".into(),
            name: "高端 福喜健 四價流感疫苗 (GC Flu Quadrivalent)".into(),
            aliases: "福喜健 / 高端流感疫苗 / GC Biopharma".into(),
            category: "Both".into(),
            target_audience: "滿 3 歲以上幼童、學童、青少年、成人與長者".into(),
            prevent_disease: "預防季節性 A 型與 B 型流行性感冒病毒感染".into(),
            full_description: "【與韓國生技大廠 GC Biopharma 合作】引進世界衛生組織 (WHO) 預認證之四價流感抗原原液，符合國際規格，適用於滿 3 歲以上之公自費流感接種。".into(),
            schedule: vec![
                "滿3歲至未滿9歲初次接種：施打 2 劑 (間隔 4 週)".into(),
                "9歲以上至成人長者：每年施打 1 劑".into(),
            ],
            notes: "適應症為滿 3 歲以上，未滿 3 歲嬰幼兒請選擇適用月齡廠牌。".into(),
        },
        VaccineDetailDoc {
            id: "covid19".into(),
...
#[derive(Serialize, Deserialize, Clone)]
pub struct CatchUpResponse {    pub vaccine_name: String,
    pub next_dose_info: String,
    pub earliest_date_display: String,
    pub days_remaining: i64,
    pub is_ready_now: bool,
    pub acip_rule_summary: String,
    pub clinical_notes: Vec<String>,
}


pub fn calculate_catch_up_core(
    vaccine_id: String,
    last_dose_num: i32,
    year: i32,
    month: u32,
...
#[derive(Serialize, Deserialize, Clone)]
pub struct TravelVaccineItem {    pub name: String,
    pub requirement_type: String, // "Mandatory" | "Recommended" | "Booster"
    pub timing_note: String,
    pub yellow_book_required: bool,
    pub description: String,
}

#[derive(Serialize, Deserialize, Clone)]
pub struct TravelAdvisoryResponse {    pub destination_name: String,
    pub purpose_name: String,
    pub mandatory_items: Vec<TravelVaccineItem>,
    pub recommended_items: Vec<TravelVaccineItem>,
    pub booster_items: Vec<TravelVaccineItem>,
    pub travel_clinic_notes: Vec<String>,
}


pub fn get_travel_advisory_core(
    destination: String,
    purpose: String,
...
#[derive(Serialize, Deserialize, Clone)]
pub struct GrowthMetricResult {    pub metric_name: String,
    pub user_val: f64,
    pub unit: String,
    pub percentile_label: String,
    pub percentile_val: f64,
    p3: f64,
    p15: f64,
    p50: f64,
    p85: f64,
...
#[derive(Serialize, Deserialize, Clone)]
pub struct GrowthResponse {    pub age_display: String,
    pub gender_display: String,
    pub data_sources_citation: Vec<String>,
    pub height_result: GrowthMetricResult,
    pub weight_result: Option<GrowthMetricResult>,
    pub bmi_result: Option<GrowthMetricResult>,
    pub head_result: Option<GrowthMetricResult>,
    pub overall_advice: Vec<String>,
}


pub fn calculate_growth_percentile_core(
    gender: String,
    age_months: i32,
    height: f64,
    weight: f64,
...
#[wasm_bindgen]
pub fn calculate_growth_percentile(
    gender: String,
    age_months: i32,
    height: f64,
    weight: f64,
